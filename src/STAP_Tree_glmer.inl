...
void STAP_Tree_glmer::Leapfrog(STAP_glmer& stap_object,SV_glmer& sv, double epsilon, Eigen::MatrixXd& Sigma_inverse){

    stap_object.calculate_gradient(sv,Sigma_inverse);

    svn.momenta_leapfrog_other(sv,epsilon,stap_object.sgg);

    svn.momenta_leapfrog_position(sv,epsilon);

    stap_object.calculate_gradient(svn,Sigma_inverse);
...
void STAP_Tree_glmer::BuildTree(STAP_glmer& stap_object,
    ...
    if( j == 0 ){
        if(diagnostics)
            Rcpp::Rcout << "Base Case Reached:" << std::endl;
        double total_energy_init = stap_object.calculate_glmer_energy(sv_init,Sigma_inverse,S,nu);
        this->Leapfrog(stap_object,sv_proposed,v*epsilon,Sigma_inverse);
        double total_energy = stap_object.calculate_glmer_energy(svn,Sigma_inverse,S,nu);
        n_prime = u <= total_energy ? 1: 0;
        s_prime = u < (1000 + total_energy) ? 1:0;
        svl.copy_SV_glmer(svn);
        svr.copy_SV_glmer(svn);
        ...
        STAP_Tree_glmer subtree(spc,diagnostics,rng);
        subtree.BuildTree(stap_object,sv_proposed, sv_init,u,v,j-1,epsilon,Sigma_inverse,S,nu,rng);
        s_prime = subtree.get_s_prime();
        n_prime = subtree.get_n_prime();
        n_alpha = subtree.get_n_alpha();
        svn.copy_SV_glmer(subtree.get_svn());
        ...
        if(subtree.get_s_prime() == 1){
            STAP_Tree_glmer subsubtree(spc,diagnostics,rng);
            if( v == -1 ){
                subsubtree.BuildTree(stap_object,svl,sv_init,u,v,j-1,epsilon,Sigma_inverse,S,nu,rng);
                svl.copy_SV_glmer(subsubtree.get_svl());
            }else{
                subsubtree.BuildTree(stap_object,svr,sv_init,u,v,j-1,epsilon,Sigma_inverse,S,nu,rng);
                svr.copy_SV_glmer(subsubtree.get_svr());
            }
            double p = (subsubtree.get_n_prime() == 0.0 && subtree.get_n_prime() ==0.0) ? 0.0 : subsubtree.get_n_prime() / (subtree.get_n_prime() + subsubtree.get_n_prime());
            std::uniform_real_distribution<double> die(0.0,1.0);
