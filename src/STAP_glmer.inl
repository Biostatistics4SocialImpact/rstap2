void STAP_glmer::calculate_glmer_eta(SV_glmer& svg){
    ...
    eta = svg.get_alpha_vector() + X_diff * svg.beta + X_mean * svg.beta_bar + Z * svg.delta + ((W.array() * (subj_array.transpose() * svg.b).array()).rowwise().sum()).matrix() ;
}
...
double STAP_glmer::calculate_glmer_ll(SV_glmer& svg){
    ...
    out += - svg.b.rows() / 2.0  * log(M_PI * 2) - .5 * log(Sigma_inverse.determinant());
    for(int i = 0; i < svg.b.rows() ; i++)
        out += - .5  * svg.b.row(i) * Sigma_inverse * svg.b.row(i).transpose();
...
double STAP_glmer::calculate_glmer_energy(SV_glmer& svg, Eigen::MatrixXd& Sigma_inverse,Eigen::MatrixXd& S, const int& nu){
    ...
    double out = 0;
    out = this->calculate_glmer_ll(svg,Sigma_inverse);
    ...
    // Wishart prior on Sigma inverse 
    //out += (nu - 2 - 1) / 2  * log(Sigma_inverse.determinant()) - .5 * (S.inverse() * Sigma_inverse).trace();
    //out += - nu * log(2) - (nu / 2.0) *  log(S.determinant()) - log(M_PI) - lgamma(nu/2) - lgamma(nu/2 - .5);
...
double STAP_glmer::sample_u(SV_glmer& svg,Eigen::MatrixXd& Sigma_inverse, Eigen::MatrixXd& S, const int& nu, std::mt19937& rng){

    if(diagnostics)
        Rcpp::Rcout << "Sample U Energy Calculation" << std::endl;
    double energy = this->calculate_glmer_energy(svg,Sigma_inverse,S,nu);
    std::uniform_real_distribution<double> runif(0.0,1.0);
    double log_z = log(runif(rng));
    return(energy + log_z);
...
void STAP_glmer::calculate_gradient(SV_glmer& svg, Eigen::MatrixXd& Sigma_inverse){
    ...
    sgg.b_grad.col(1) = precision * (subj_array * ((y-eta).array() * W.col(1).array()).matrix());
    for(int i=0; i< sgg.b_grad.rows(); i++)
        sgg.b_grad.row(i) = sgg.b_grad.row(i) - Sigma_inverse * svg.b.row(i);
    //sgg.b_grad = sgg.b_grad - (Sigma_inverse * svg.b.transpose()).transpose();
...
double STAP_glmer::FindReasonableEpsilon(SV_glmer& sv, std::mt19937& rng, const int& nu){
    ...
    SV_glmer sv_prop(sv.spc,rng,diagnostics);
    double ratio,initial_energy,propose_energy;
    initial_energy = this->calculate_glmer_energy(sv,Sigma_inverse,S,nu);
    this->calculate_gradient(sv,Sigma_inverse);
    sv_prop.momenta_leapfrog_other(sv,epsilon,sgg);
    sv_prop.momenta_leapfrog_position(sv,epsilon);
    this->calculate_gradient(sv_prop,Sigma_inverse);
    sv_prop.momenta_leapfrog_self(epsilon,sgg);
    propose_energy = this->calculate_glmer_energy(sv_prop,Sigma_inverse,S,nu);
    ratio =  propose_energy - initial_energy;
    if(diagnostics)
        Rcpp::Rcout << "ratio calc" << propose_energy << " " << initial_energy << " " << propose_energy - initial_energy << std::endl;
    ratio = isinf(-ratio) ? -DBL_MAX: ratio ;
