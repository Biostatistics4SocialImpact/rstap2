---
title: "STAP DND Mixed Effects - Random Intercepts"
...
X <- dists %>% group_by(id,time) %>% 
    summarise(Exposure = sum(exp(-Distance/theta_s)))
...
customthetas <- seq(from = X_bar$subj_int[2] - 1, to= X_bar$subj_int[2] + 1, by = 0.05)
sink("~/Desktop/Routput.txt")
...
tibble(theta = (customthetas), energy = out$energy) %>% ggplot(aes(x=theta,y=energy)) + geom_line() + theme_bw()  + geom_vline(aes(xintercept = X_bar$subj_int[2]) ,linetype = 2) 

tibble(theta = (customthetas), grad = out$grad) %>% 
    ggplot(aes(x=theta,y=grad)) + geom_line() + theme_bw()  +
    geom_vline(aes(xintercept = X_bar$subj_int[2]),linetype = 2) 
...
```{r}
samples %>% filter(acceptance==1,ix>300) %>% 
    gather(delta,beta,beta_bar,theta,sigma,alpha,Sigma,key="Parameters",value="Samples") %>% 
    mutate(Truth = (Parameters=="beta")*beta + (Parameters =="theta")*theta_s + (Parameters=="sigma")*sigma + 
           (Parameters=="delta") *delta +  (Parameters=="beta_bar")*beta_bar + (Parameters=="Sigma")*sigma_b + 
...
samples %>% filter(acceptance==1,ix>300) %>% 
    ...
    mutate(Truth = (Parameters=="beta")*beta + (Parameters =="theta")*theta_s + (Parameters=="sigma")*sigma + 
           (Parameters=="delta") *delta +  (Parameters=="beta_bar")*beta_bar + (Parameters=="Sigma")*sigma_b + 
           (Parameters=="alpha")*alpha) %>% 
    ggplot(aes(x=ix,y=Samples)) + geom_line() + theme_bw() + 
    geom_vline(aes(xintercept=Truth),linetype=2) +
    facet_wrap(~Parameters,scales="free") + ggtitle("Custom NUTS - Posterior Samples") + 
    theme(strip.background = element_blank())
...
```{r}
b_mns <- colMeans(fit$b_samps[which(fit$acceptance == 1),])
tibble(b_hat = b_mns,
       true_b = X_bar$subj_int) %>% 
ggplot(aes(x=b_hat,y=true_b)) + geom_point() + geom_abline(intercept = 0) + theme_bw() + ggtitle("Estimates of true vs. estimated subject intercepts") + ggsave("~/Desktop/b_ests.png",width=7,height=5)
