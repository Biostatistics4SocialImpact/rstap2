...
W <- matrix(1,nrow = num_subj,ncol=1)
```
...
fit <- stapdnd_glmer(y = y, Z = Z,W = W,
                     u_crs = as.matrix(u_crs),
                      ...
                      stap_par_code = c(length(y),1,1,1,num_subj,1),
                      distances = dists_crs,
                      adapt_delta = .65,
                      warmup = warmup, 
                      iter_max = iter_max,
...
```{r}
median_bs <- apply(fit$b_samps,2,median)
...
```{r,echo = F}
# sigma_bs <- log(seq(from = 0.01, to =2, by = 0.05 ))
# 
# sink("~/Desktop/Routput.txt")
# out <- test_grads_glmer(y,Z,W,
#                         true_b = matrix(X_bar$subj_int,ncol=1),
#                         beta_bar,beta,
#                         dists_crs,as.matrix(u_crs),
#                         subj_mat1,as.matrix(subj_n,ncol=1),
#                         sigma_bs,
#                         c(length(y),1,1,1,num_subj,1),
#                         seed = 1241)
# sink()
# 
# tibble(subj_sigma = exp(sigma_bs),
#        energy = out$energy) %>% 
#     ggplot(aes(x=subj_sigma,y=energy)) + geom_line() + theme_bw()  + geom_vline(aes(xintercept = 1.5) ,linetype = 2) 
...
# tibble(delta = deltas,
#        grad = out$grad) %>% 
#     ggplot(aes(x=delta,y=grad)) + geom_line() + theme_bw()  + geom_vline(aes(xintercept = -.5) ,linetype = 2) 
```
