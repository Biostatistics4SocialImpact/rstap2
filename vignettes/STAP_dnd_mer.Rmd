...
```{r}
thetas <- seq(from = -5, to =2.0, by =0.05);
sink("~/Desktop/Routput.txt")
out <- test_grads_glmer(y,Z,W,as.matrix(X_bar$subj_int,ncol=1),beta_bar,beta,
                  dists_crs,as.matrix(u_crs),
                  subj_mat1,as.matrix(subj_n,ncol=1),thetas,
                  c(length(y),1,1,1,num_subj),
                  seed = 1241)
sink()

transform_thetas <- function(x) 10 / (1 + exp(-x))
tibble(theta = transform_thetas(thetas), energy = out$energy) %>% ggplot(aes(x=theta,y=energy)) + geom_line() + theme_bw()  + geom_vline(aes(xintercept = .5),linetype = 2) 

tibble(theta = transform_thetas(thetas), grad = out$grad) %>% ggplot(aes(x=theta,y=grad)) + geom_line() + theme_bw()  + geom_vline(aes(xintercept = .5),linetype = 2) 
