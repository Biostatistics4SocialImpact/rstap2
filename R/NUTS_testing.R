...
y <- beta*X_diff + rnorm(n = num_subj*3,
                         mean = 0,
                         sd = sigma)
y_stapreg <- beta*X + rnorm(n = num_subj*3,mean = 0, sd = 1)
beta_init <- rnorm(1)
theta_init <- rnorm(1)
...
fit <- test_grads(y = y,
                      ...
                      adapt_delta = .85,
                      theta_grid = seq(from = -4, to = 0, by =0.05),
...
rslts %>% filter(theta<=-2.8) %>%  
    ggplot(aes(x=beta,y=energy,color=factor(theta))) + 
    geom_line() + theme_bw() + ggtitle("Energy - beta perspective")

rslts  %>% mutate(transformed_theta = 10/(1.0 + exp(-theta))) %>%  
    filter(beta>1,beta<=1.3) %>% 
    ggplot(aes(x=transformed_theta,y=energy,color=factor(beta))) + 
    geom_line() + theme_bw() + ggtitle("Energy - theta perspective")
...
rslts %>% mutate(transformed_theta = 10/(1.0 + exp(-theta))) %>% 
    ...
    geom_line() + theme_bw() + ggtitle("Theta gradient - CPP Analytic")
...
rslts %>% filter(theta %in% c(-1.9,-2.95,-2)) %>%
